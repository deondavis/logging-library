...
namespace logging
{
    ...
    public:
        ...
        virtual void log(const std::string &message, const log_level level)
        {
            if (level < LOG_LEVEL_CUTOFF)
                return;
            std::string output;
            output.reserve(message.length() + LOG_HEADDER);
    ...
    class file_logger : public logger
    {
        ...
        void reopen(void)
        {
            //check if the file is to be reopened
    ...
    inline void configure(const logging_config_t& config)
    {
     ...
     //configure the singleton from file
    void configure_from_file(std::string config_file)
    {
        ...
        config["type"] = root.get("type", "UTF-32" ).asString();
        config["file_name"] = root.get("file_name", "UTF-32" ).asString();
        config["reopen_interval"] = root.get("reopen_interval", "UTF-32" ).asString();
        config["color"] = root.get("color", "UTF-32" ).asString();
        get_logger(config);
    }
